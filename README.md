...
本仓库已经公开，用于展示 TaskDeck 的设计、实现与持续迭代过程。仓库只包含源码、测试、文档和不含个人数据的演示材料；你的任务数据库、专注记录、JSON 导出、自动备份、签名证书和本机发布配置均不会被 Git 跟踪。

- 发现普通问题或希望提出功能建议，请使用 [GitHub Issues](https://github.com/USYDDonghaoLi/TaskDeck/issues)。
- 安全漏洞或可能涉及隐私的数据泄露，请按照 [SECURITY.md](SECURITY.md) 私下报告，不要创建公开 Issue。
- Public 仅代表源码可见。仓库目前尚未选择开源许可证，因此不自动授予复制、修改、分发或商业使用权。

## 功能

- 使用“大方向 + 精准任务描述 + 预计时间”创建任务
- 为任务拆分可单独勾选的子任务，重复任务会复制一套未完成步骤
- 通过任务卡片上的铅笔编辑名称、预计时长、优先级、备注和日程
- 支持每天、工作日、每周和每月重复任务
- 一键延期到一小时后、明天或下周一
- 通过 macOS 系统通知发送提醒
- 针对具体任务开始、暂停、继续和结束专注计时
- 结束专注时可选择仅停止、完成任务或切换下一项，并可记录本次完成内容
- 专注开始后可在屏幕顶部显示悬浮控制条，可在设置中选择始终显示、划过顶部把手时展开或完全隐藏
- 可选番茄钟支持自定义专注/休息时长、轮次提醒和自动开始休息
- 闲置返回时询问是否计入离开期间，避免虚高的专注报告
...
- 支持 `⌘N` 新建、`⌘F` 搜索、`⌘⌥F` 清除筛选、`⌘1/2/3` 切换任务流、`⌘⇧D` 打开桌面板、`⌘⌥K` 打开专注任务切换器和 `⌘⌥J` 切换到最近任务
...
1.6 又增加了旧开发 App Group 到正式发布 App Group 的只读 SQLite 迁移：正式版首次启动会复制任务、专注历史和运行中计时，旧数据库原样保留。1.7 的删除操作改为回收站，并提供经过完整性校验的数据库备份恢复；升级时只新增可空字段，不会删除已有任务。1.8 启用早已预留的子任务关系表，因此不需要破坏性迁移。1.9 不改变数据库结构；XCTest 始终使用临时目录，新手引导也不会读取或修改任务。2.3 仅为专注记录新增可空的总结字段，迁移前会自动备份数据库；番茄钟、闲置检测和最近任务偏好只保存在本机。完整隐私声明见 [PRIVACY.md](PRIVACY.md)。
...
4. 点击悬浮条的“结束专注”可仅停止、完成任务或切换下一项；也可以填写本次成果。
5. 使用 `⌘⌥K` 打开带搜索和方向分组的切换器，使用 `⌘⌥J` 快速切回最近任务。
6. 点击任务卡片上的铅笔，可修改名称、预计时长和优先级。
7. 点击右上角“设置”，可切换语言，并将顶部专注悬浮窗设为“始终显示”“划过时显示”或“不显示”；划过模式平时只保留屏幕顶部的小把手，鼠标移入后展开完整控制条。
8. 设置中还可配置番茄钟与闲置检测、管理回收站、浏览/恢复自动备份，以及导入/导出 JSON。
9. 从右侧报告面板查看日、周、月战绩或导出 PDF。
10. 点击“桌面悬浮”打开常驻桌面的迷你任务板。
11. 在桌面空白处右键“编辑小组件”，搜索 TaskDeck，并选择小号、中号或大号。
...
当前版本：TaskDeck 2.3.1（Build 17）。

这是一个持续迭代的个人 Vibe Coding 项目。公开仓库不包含作者或用户在 TaskDeck 中创建的任务数据；每次提交和发布也会经过自动隐私检查。
