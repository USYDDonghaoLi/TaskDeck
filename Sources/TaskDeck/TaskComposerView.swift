...
struct TaskComposerView: View {
        ...
        HStack(alignment: .top) {
            VStack(alignment: .leading, spacing: 5) {
                Text(editingTask == nil ? "INJECT NEW TASK" : "PATCH TASK NODE")
                    ...
                    .tracking(1.7)
                Text(editingTask == nil ? "创建精准任务" : "编辑精准任务")
                    .font(.system(size: 21, weight: .black))
                Text(editingTask == nil ? "方向决定为什么做，描述明确下一步做什么。" : "任务 ID 与历史记录保持不变。")
                    .font(.system(size: 10))
    ...
    private var directionSection: some View {
        VStack(alignment: .leading, spacing: 8) {
            FieldLabel(number: "01", text: "大方向")
            TextField("例如：产品发布 / 健身 / 学习", text: $direction)
                .deckTextField()
    ...
    private var descriptionSection: some View {
        VStack(alignment: .leading, spacing: 8) {
            FieldLabel(number: "02", text: "精准任务描述")
            ZStack(alignment: .topLeading) {
                if title.isEmpty {
                    Text("用动词开头，例如：整理反馈并输出 3 条改版结论")
                        .font(.system(size: 11))
    ...
    private var durationSection: some View {
        VStack(alignment: .leading, spacing: 9) {
            FieldLabel(number: "04", text: "预计时间")
            HStack(spacing: 7) {
            ...
            Stepper("自定义：\(estimatedMinutes) 分钟", value: $estimatedMinutes, in: 5...480, step: 5)
    ...
    private var schedulingSection: some View {
        VStack(alignment: .leading, spacing: 11) {
            Toggle(isOn: $hasDueDate) {
                FieldLabel(number: "05", text: "计划与重复")
            }
            .toggleStyle(.switch)
            ...
            if hasDueDate {
                ...
                HStack {
                    Text("重复规则")
                        .font(.system(size: 10, weight: .medium))
                    Spacer()
                    Picker("重复规则", selection: $recurrence) {
                ...
                Toggle("到时间发送系统通知", isOn: $reminderEnabled)
                    .toggleStyle(.switch)
                    .font(.system(size: 10, weight: .medium))

                if notifications.authorizationStatus == .denied {
                    Label("通知权限已关闭，可在“系统设置 → 通知 → TaskDeck”中开启", systemImage: "exclamationmark.triangle")
    ...
    private var actionBar: some View {
        HStack(spacing: 10) {
            ...
            Button(action: saveTask) {
                Label(editingTask == nil ? "写入任务队列" : "保存任务修改", systemImage: editingTask == nil ? "arrow.down.to.line" : "checkmark")
